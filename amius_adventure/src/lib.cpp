...
Engine::Engine(std::string platform, void(*softPanic)(std::string), Scene::Scene* topScene, Scene::Scene* bottomScene, AssetProviderInterface* assetProvider): platform(platform), softPanic(softPanic), assetProvider(assetProvider) {
    ...
    topScene->ctx.camera->position.x = 0;
    topScene->ctx.camera->position.y = 0;
    topScene->ctx.camera->position.z = 6;
    ...
    topScene->objects[1] = NEW_MODEL(assetProvider->getAssetLocation("/models/3ds_test_model", MODEL_ASSET_TYPE), assetProvider->getAssetLocation("/gfx/3ds_test_model", TEXTURE_ASSET_TYPE), -0.25, 0, -3.0, 0, 0, 0, 0.5, 0.5, 0.5, /*rotateCubeTick*/ nullptr);
    // no assets yet
    // topScene->objects[2] = NEW_3D_SPRITE("romfs:/gfx/marioTex.t3x", 1, 1, 64, 64, 41, 25, 250, 12, 0, 0, 0, 1, 1, frontPlayerTick);

    topScene->objects[3] = NEW_EMPTY(0, 0, 0, 0, 0, 0, 1, 1, 1, playMusic);
